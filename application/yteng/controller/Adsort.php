...
use sort\Sortclass;
use think\Db;
use app\yteng\model\Adsortclass as Sor;
class Adsort extends Par{
	protected $table = "adsortclass";
	...
	public function save(){
		...
		if($data["id"]){
			...
			if($count){
				ajson(["status"=>0,"mes"=>"同广告位置下名称不能重复"]);
			}
			else{
			...
			if($count){
				...
				Sor::where('id', $col->id)->update(['sortpath' => $sortpath]);
				$info = $this->getSlide();
